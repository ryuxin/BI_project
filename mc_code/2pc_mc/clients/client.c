...
disconnect_mc_server(int me, int server, mc_message_t type)
{
	...
	msg       = (struct mc_msg *)rpc_addr(me, server, msg_idx[me][server]++);
	msg->type = type;
	msg->id   = me;
...
static int client_get_key(char *key, int nkey)
{
	...
	hv           = hash(key, nkey);
//	node         = hv2node(hv);
	node         = cur_node - NUM_NODE/2;
	msg          = (struct mc_msg *)rpc_addr(cur_node, node, msg_idx[cur_node][node]++);
