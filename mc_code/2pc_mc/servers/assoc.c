...
assoc_init(void *addr)
{
    ...
    sz  = hashsize(HASHPOWER_DEFAULT) * sizeof(struct list_head);
    ret = posix_memalign((void **)&primary_hashtable, PS_PAGE_SIZE, sz);
    assert(!ret);
    memset(primary_hashtable, 0, sz);
    return ;
}
...
uint32_t
assoc_evict(uint32_t hv)
{
    uint32_t i;

    i = ((hv+1)%(hashsize(HASHPOWER_DEFAULT) - 1)) + 1;
    while (i != hv) {
        if (primary_hashtable[i].head) break;
        i = ((i+1) % (hashsize(HASHPOWER_DEFAULT) - 1)) + 1;
    }
    return i;
}

item *
assoc_hv2it(uint32_t hv)
{  return primary_hashtable[hv].head; }
