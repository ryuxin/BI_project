...
#ifdef KEY_SKEW
extern struct ps_slab *mc_alloc_pages(struct ps_mem *m, size_t sz, coreid_t coreid);
extern void mc_free_pages(struct ps_mem *m, struct ps_slab *s, size_t sz, coreid_t coreid);
PS_SLAB_CREATE_AFNS(item, MC_SLAB_OBJ_SZ, MC_SLAB_AOC_SZ, sizeof(struct ps_slab), mc_alloc_pages, mc_free_pages)
#else
PS_SLAB_CREATE(item, MC_SLAB_OBJ_SZ, MC_SLAB_AOC_SZ)
#endif


int do_item_size(void)
{
    return ps_slab_objmem_item();
}
                    ...
                    const uint32_t cur_hv)
{
    ...
    it = ps_slab_alloc_item();
    if (it == NULL) it = ps_slab_alloc_item();
    if (it == NULL) {
...
void
do_item_unlink(item *it, const uint32_t hv)
{
...
do_item_replace(item *old, item *new, const uint32_t hv)
{
    ...
    assoc_delete(ITEM_key(old), old->nkey, hv);
...
item *
do_item_touch(const char *key, size_t nkey, uint32_t exptime, const uint32_t hv)
{
